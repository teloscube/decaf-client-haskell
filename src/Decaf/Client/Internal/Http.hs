...
-- | Runs a request and returns 'B.ByteString'.
runRequest :: IT.Request -> IO B.ByteString
runRequest r = HS.getResponseBody <$> (HS.httpBS . compileRequest) r


-- | Runs a request, decodes the response body into a value and returns it.
runRequest' :: FromJSON a => IT.Request -> IO a
runRequest' r = HS.getResponseBody <$> (HS.httpJSON . compileRequest) r
