...
mkBaristaClient :: H.BaseUrl -> H.Credentials -> BaristaClient
mkBaristaClient baseUrl credentials = MkBaristaClient request
  where
    auth = H.mkAuthorization credentials
    request = H.setEndpoint baristaEndpoint $ H.mkRequest baseUrl auth []
...
setEndpoint :: H.Endpoint -> BaristaClient -> BaristaClient
setEndpoint = modifyClient . H.setEndpoint
...
addPath :: H.Path -> BaristaClient -> BaristaClient
addPath = modifyClient . H.addPath
...
prepareGet :: H.Params ->  H.Path -> H.Headers -> BaristaClient -> BaristaClient
prepareGet params path headers = addParams params . setEndpoint (H.Endpoint H.GET path) . addHeaders headers

-- | Attempt to perform a GET request.
get :: H.Params ->  H.Path -> H.Headers -> BaristaClient -> IO B.ByteString
