...
let
  ...
  ## Get this package:
  thisPackage = haskell.callCabal2nixWithOptions "decaf-client" ../. "" { };
  ...
  thisPackageInstallable = pkgs.haskell.lib.dontCheck (pkgs.haskell.lib.justStaticExecutables thisPackage);
in
{
  sources = sources;
  telosnix = telosnix;
